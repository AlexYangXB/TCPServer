...
namespace NodeServerAndManager.Control
{
    ...
    public partial class IpControl : UserControl
    {
        ...
        private void IpControl_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            int SelectedTextLength = ((TextBox) sender).SelectedText.Length;
            ...
            if (Regex.Match(KeyChar.ToString(), "[0-9]").Success)//keyChar 为数字0~9
            {
                if (((TextBox)sender) == textBox1 && KeyChar == '0' && (TextLength == 0 || SelectedTextLength == TextLength))//第一个文本框的第一位不能输入0
                {
                    e.Handled = true;
                }
                //当文本框内文本长度为2，且文本框内的文本没有选中
                else if (TextLength == 2 && ((TextBox)sender).SelectedText.Length==0)
                {
                ...
                else if (TextLength == 3 && SelectedTextLength == 0)//当长度达到3，且没有文本被选中时，不可再输入
                {
                    e.Handled = true;
                }
                else if (TextLength == 1 && SelectedTextLength == 0 && ((TextBox)sender).Text == "0")//当长度为1，且文本为0时，用当前值替换0
                {
                    ((TextBox) sender).Text = "";
                }
            }
