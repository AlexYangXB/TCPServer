            ...
            set
            {
                ...
                for(int i=0;i<dt.Rows.Count;i++)
                {
                    if (!machine.ContainsKey(dt.Rows[i]["kIpAddress"].ToString()))
                    {
                        ...
                        data.nodeId = (int)dt.Rows[i]["kNodeId"];
                        data.factoryId = (int)dt.Rows[i]["kFactoryId"];
            ...
            while (true)  // 持续不断的监听客户端的连接请求；  
            {
                ...
                else
                {
                    ...
                    if(dt.Rows.Count>0)
                    {
                        ...
                        data.nodeId = (int)dt.Rows[0]["kNodeId"];
                        data.factoryId = (int)dt.Rows[0]["kFactoryId"];
                    ...
                    if (length == 4 && BitConverter.ToInt32(readBuf, 0) == 0x4C4A4040)
                    {
                        ...
                        if (length == 4)
                        {
                            ...
                            else if (cmd > 0 && cmd < 10)
                            {
                                ...
                                //TimeSpan sp = machine[ip[0]].dateTime - fileTime;
                                if (machine[ip[0]].startBusinessCtl && machine[ip[0]].dateTime<fileTime)
                                {
                                ...
                                else//没有进行交易控制
                                {
                                    ...
                                    //string machineNum = "",machineModel;
                                    Utility.SaveDataToDB.SaveFsn(fileName, PictureServerId, machine[ip[0]],false);
                                    if (machineNum != machine[ip[0]].machineNumber)
                                        machine[ip[0]].machineNumber = machineNum;
                                }
            ...
            switch (myBusinessStatus)
            {
                case MyBusinessStatus.Start:
                    if(machine.ContainsKey(bControl.ip))
                    {
                        ...
                        machine[bControl.ip].userId = bControl.userId;
                ...
                case MyBusinessStatus.End:
                    ...
                    if (machine.ContainsKey(bControl.ip))
                    {
                        ...
                        {
                            string machineNum = "",machineModel="";
                            Utility.SaveDataToDB.SaveFsn(machine[bControl.ip].fileName, PictureServerId, machine[bControl.ip],true, out machineNum,out machineModel);
                            if (machine[bControl.ip].machineNumber != machineNum)
                                machine[bControl.ip].machineNumber = machineNum;
                        }
                ...
                case MyBusinessStatus.Cancel:
                    if (machine.ContainsKey(bControl.ip))
                    {
                        ...
                        {
                            string machineNum = "",machineModel="";
                            Utility.SaveDataToDB.SaveFsn(machine[bControl.ip].fileName, PictureServerId, machine[bControl.ip], false, out machineNum, out machineModel);
                            if (machine[bControl.ip].machineNumber != machineNum)
                                machine[bControl.ip].machineNumber = machineNum;
                        }
                        machine[bControl.ip].fileName = "";
