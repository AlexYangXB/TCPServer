...
namespace KyData.DataBase
{
    public class KYDataLayer1
    {
        ...
        /// DAT(交行格式文件转FSN格式)
        /// </summary>
        /// <param name="bDat">DAT数据</param>
        /// <param name="bFsn">FSN数据</param>
        /// <param name="business">业务类型</param>
        /// <param name="currency">币种</param>
        public static void DatToFsn(byte[] bDat, out byte[] bFsn, out string business, out string currency)
        {
            business = "K";
            currency = Encoding.ASCII.GetString(bDat, 30, 3).Replace('\0', ' ').Replace(" ", "").Trim(); ;

            UInt16 Date;		  	//验钞启动日期
            UInt16 Time;		  	//验钞启动时间
            UInt16 tfFlag;		//真、假、残和旧币标志
            UInt16[] ErrorCode = new ushort[3];  //错误码(3个)
            UInt16[] MoneyFlag = new ushort[4]; 	//货币标志
            UInt16 Ver;           //版本号
            UInt16 Valuta;		//币值
            UInt16 CharNUM;	  	//冠字号码字符数
            UInt16[] SNo = new ushort[12];		//冠字号码
            UInt16[] MachineSNo = new ushort[24];//机具编号
            UInt16 Reserve1 = 0;       //保留字1
            //UInt16 ImageSNo;//图像冠字号码
            UInt16 Num;
            UInt16 Height = 32;
            UInt16 Width = 32;
            UInt16 Reserve2 = 0;
            ...
            string dateTime = Encoding.ASCII.GetString(bDat, 0, 14);
            string date = dateTime.Substring(0, 8);
            string time = dateTime.Substring(8);
            Date = (UInt16)(((int.Parse(date.Substring(0, 4)) - 1980) << 9) + (int.Parse(date.Substring(4, 2)) << 5) + int.Parse(date.Substring(6, 2)));
            Time = (UInt16)(((int.Parse(time.Substring(0, 2)) << 11) + (int.Parse(time.Substring(2, 2)) << 5) + (int.Parse(time.Substring(4, 2)) >> 1)));
            if (Encoding.ASCII.GetString(bDat, 39, 1) == "0")
            {
                tfFlag = 1;
            }
            else
            ...
            {
                if (bDat[30 + j] != '\0')
                    MoneyFlag[j] = bDat[30 + j];
            }
            string version = Encoding.ASCII.GetString(bDat, 26, 4).Replace('\0', ' ').Replace(" ", "").Trim();
            if (version == "1990")
            {
                Ver = 0;
            }
            else if (version == "1999")
            {
                Ver = 1;
            }
            ...
            Valuta = UInt16.Parse(Encoding.ASCII.GetString(bDat, 33, 6));
            byte[] bSign = Encoding.ASCII.GetBytes(Encoding.ASCII.GetString(bDat, 14, 12).Replace('\0', ' ').Replace(" ", "").Trim());
            for (int j = 0; j < bSign.Length; j++)
            {
                SNo[j] = bSign[j];
            }
            CharNUM = (UInt16)bSign.Length;
            byte[] bMachine = Encoding.ASCII.GetBytes(Encoding.ASCII.GetString(bDat, 41, 24).Replace('\0', ' ').Replace(" ", "").Trim());
            for (int j = 0; j < bMachine.Length; j++)
            {
                ...
                {
                    MachineSNo[j] = bMachine[j + 1];
                }

            }
            CopyMemoryEx(imageData, bDat, 96, bDat.Length - 96);

            bFsn = new byte[1644];
            BitConverter.GetBytes(Date).CopyTo(bFsn, 0);
            BitConverter.GetBytes(Time).CopyTo(bFsn, sizeof(ushort));
            BitConverter.GetBytes(tfFlag).CopyTo(bFsn, 2 * sizeof(ushort));
            for (int j = 0; j < 3; j++)
            {
                BitConverter.GetBytes(ErrorCode[j]).CopyTo(bFsn, (3 + j) * sizeof(ushort));
            }
            for (int j = 0; j < 4; j++)
            {
                BitConverter.GetBytes(MoneyFlag[j]).CopyTo(bFsn, (6 + j) * sizeof(ushort));
            }
            BitConverter.GetBytes(Ver).CopyTo(bFsn, 10 * sizeof(ushort));
            BitConverter.GetBytes(Valuta).CopyTo(bFsn, 11 * sizeof(ushort));
            BitConverter.GetBytes(CharNUM).CopyTo(bFsn, 12 * sizeof(ushort));
            for (int j = 0; j < 12; j++)
            {
                BitConverter.GetBytes(SNo[j]).CopyTo(bFsn, (13 + j) * sizeof(ushort));
            }
            for (int j = 0; j < 24; j++)
            {
                BitConverter.GetBytes(MachineSNo[j]).CopyTo(bFsn, (25 + j) * sizeof(ushort));
            }
            BitConverter.GetBytes(Reserve1).CopyTo(bFsn, 49 * sizeof(ushort));
            BitConverter.GetBytes(CharNUM).CopyTo(bFsn, 50 * sizeof(ushort));
            BitConverter.GetBytes(Height).CopyTo(bFsn, 51 * sizeof(ushort));
            BitConverter.GetBytes(Width).CopyTo(bFsn, 52 * sizeof(ushort));
            BitConverter.GetBytes(Reserve2).CopyTo(bFsn, 53 * sizeof(ushort));
            imageData.CopyTo(bFsn, 54 * sizeof(ushort));
        }

        /// <summary>
        /// 
        /// </summary>
        /// <param name="sourceFsn"></param>
        /// <param name="bFsn"></param>
        /// <param name="business">业务类型（取、存、配、空）（Q、C、P、K）</param>
        /// <param name="currency">币种 </param>
        /// <returns></returns>
        public static bool FsnToFsn(byte[] sourceFsn, out byte[] bFsn, out string business, out string currency)
        {
            currency = "";
            business = "K";
            bool isChange = false;
            ...
            byte[] businessByte = new byte[24];
            //币种
            currency = Encoding.ASCII.GetString(sourceFsn, 12, 8).Replace('\0', ' ').Replace(" ", "");
            //机具编号
            Array.Copy(sourceFsn, 50, machineMacByte, 0, 48);
            ...
            string machineMac = Encoding.ASCII.GetString(mac, 0, 24);
            string strHead = Encoding.ASCII.GetString(businessByte, 0, 3).Replace('\0', ' ').Replace(" ", "").Trim();
            UInt16[] MachineSNo = new ushort[24];//机具编号

            if (machineMac.Substring(0, 4) == "GZKY")
            {
                string desMachineMac = string.Format("BOC{0}/{1}", machineMac.Substring(4, 7),
                                                     machineMac.Substring(11, 13));
                byte[] byteMcahineMac = Encoding.ASCII.GetBytes(desMachineMac);
                for (int i = 0; i < byteMcahineMac.Length; i++)
                {
                    MachineSNo[i] = byteMcahineMac[i];
                }
                Array.Copy(sourceFsn, 0, bFsn, 0, 50);
                for (int j = 0; j < 24; j++)
                {
                    BitConverter.GetBytes(MachineSNo[j]).CopyTo(bFsn, (25 + j) * sizeof(ushort));
                }
                Array.Copy(sourceFsn, 98, bFsn, 98, 1644 - 98);
                isChange = true;
            }
            else if (strHead != "")
            {
                byte[] byteMcahineMac = Encoding.ASCII.GetBytes(machineMac);
                for (int i = 0; i < byteMcahineMac.Length; i++)
                {
                    MachineSNo[i] = byteMcahineMac[i];
                }
            ...
            {
                Array.Copy(sourceFsn, 0, bFsn, 0, 1644);
            }
            if (strHead == "ABC")
            {
                int businessType = businessByte[3];
        ...
        /// <summary>
        /// 标准FSN转换成含网点号、分行号的FSN
        /// </summary>
        /// <param name="sourceFsn"></param>
        /// <param name="nodeNumber">网点号 </param>
        /// <param name="branchNumber">分行号 </param>
        /// <param name="bFsn"></param>
        /// <returns></returns>
        public static bool FsnToFsn(byte[] sourceFsn, string nodeNumber, string branchNumber, out byte[] bFsn)
        {
            bool isChange = false;
            bFsn = new byte[1644];
            byte[] machineMacByte = new byte[48];
            ...
            if (strHead == "ABC" || nodeNumber == "" || branchNumber == "")
            {
                Array.Copy(sourceFsn, 0, bFsn, 0, 1644);
            }
            else
            {
                byte[] head = Encoding.ASCII.GetBytes("ABC");
                byte[] node = Encoding.ASCII.GetBytes(nodeNumber);
                byte[] branch = Encoding.ASCII.GetBytes(branchNumber);
                for (int i = 0; i < 16; i++)
                {
                    if (i < 3)
                    {
                        businessByte[i] = head[i];
                    }
                    else if (i > 3 && i < 10)
                    {
                        businessByte[i] = branch[i - 4];
                    }
                    else if (i > 9)
                    {
                        businessByte[i] = node[i - 10];
                    }
                }
                for (int i = 0; i < 24; i++)
                {
                    machineMacByte[i * 2 + 1] = businessByte[i];
                }
                Array.Copy(sourceFsn, 0, bFsn, 0, 50);
                Array.Copy(machineMacByte, 0, bFsn, 50, 48);
                Array.Copy(sourceFsn, 98, bFsn, 98, 1644 - 98);
            }
            return isChange;
        }
