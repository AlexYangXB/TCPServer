            ...
            if(success)
            {
                ...
                if (zipFile.Length > 0)//说明还有压缩文件，gzhZip为二次压缩文件
                {
                    for (int i = 0; i < zipFile.Length; i++)
                    {
                        ...
                        if (success)
                        {
                            bool result = SaveDataToDB.UploadGzhPackage(targetDirectory + "\\tmp",
                ...
                {
                    return SaveDataToDB.UploadGzhPackage(targetDirectory, pictureServerId, userId);
                }
            }
            else
            {
